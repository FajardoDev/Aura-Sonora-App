...
export default function GridPodcastCategory({
  ...
  const onPullToRefresh = async () => {
    ...
    queryClient.invalidateQueries({
      queryKey: ["radioscat", "infinite", category],
    });
  ...
  const renderFooter = () => {
    ...
    if (!hasNextPage && !isSearching && radios.length > 0) {
      return (
    ...
    return (
      <View className="flex-1 items-center justify-center py-20">
        {isFetching ? (
          ...
          <ThemedText className="">No hay radio en esta categoría.</ThemedText>
        )}
      </View>
    );
  }
